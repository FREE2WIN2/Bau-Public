...
public class WorldGUI implements TabExecutor, Listener {
	...
	public static void openMain(Player p) {
		GUI mainGUI = new GUI(GUIType.CHEST, 27, MessageHandler.getInstance().getString(p, "worldGUI_title"));
		ItemBuilder.build(s->{s.getPlayer().closeInventory(); return true;}, Material.WOODEN_HOE	,1, ItemType.DEFAULT, "ItemName");
			HeadItem item = new HeadItem(Material.PLAYER_HEAD, "world", 1);
		item.setExecutor(s -> {
			s.getPlayer().sendMessage(s.getClicked().getItemMeta().getDisplayName());
			s.getPlayer().closeInventory();
			return true;
		});
		item.setCustomSkull(CustomHead.GLOBE);
